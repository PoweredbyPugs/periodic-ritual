...
// Periodic Ritual: reflection queue picker. Items are {container, file}
// pairs — pending reflections only. Shows the period note's name so the
// queue reads as "what's waiting", not just which container.
class PRReflectQueueModal extends FuzzySuggestModal {
...
function dr_makeQuestion(text) {
    return {
        ...
        // from phaseGateField on that container's current note. Lets one daily
        // question rotate by lunar phase, pulling per-cycle wording from the
        // arc note.
        phaseGate: false,
        ...
        outputFieldName: "",
        outputFieldType: "inline",
        // Alignment bank only — where the output field is written. Reflection
        // bank ignores all three and always writes to the active note.
        //   outputTargetMode: "active" (default) | "note" | "container-current"
        //                     | "container-previous" | "recent-daily"
        //   outputTargetNote        — vault path, used when mode === "note"
        //   outputTargetContainerId — container id, used for the container modes
        outputTargetMode: "active",
        outputTargetNote: "",
        outputTargetContainerId: "",
        omitFromLLM: false,
        omitFromCollected: false,
        // Image attachment. imageMode: "" (none) | "static" | "note" | "folder".
        // imagePath meaning depends on mode:
        //   static -> path to an image file in the vault
        //   note   -> path to a .md note; first embedded image is used
...
const DR_IMAGE_EXTS = ["png", "jpg", "jpeg", "gif", "webp", "svg", "avif", "bmp"];
function dr_isImageFile(name) {
    if (!name) return false;
    const ext = name.split(".").pop().toLowerCase();
    return DR_IMAGE_EXTS.includes(ext);
}
...
const DR_DEFAULT_SETTINGS = {
    ...
    autoGenerateDailyNote: false,
    // When on, Daily Align / Daily Reflect launched from a note that isn't a
    // daily note retarget to today's daily note (created if needed). Launched
    // from inside a daily note, that note is used. Lets a homepage button
    // ("today's reflection") write to today instead of the homepage.
    targetTodaysDailyNote: false,
    imageGalleryFolder: "",
};
...
class DRReflectionModal extends Modal {
    ...
    constructor(app, questions, injectedResolved, onSubmit, options = {}) {
        ...
        // imagePaths is a parallel array (same length as questions). Each entry
        // is a resource URL string (or empty). Resolved upstream via
        // DailyRitualModule.loadImagePathsFor so the modal stays synchronous.
        this.imagePaths = Array.isArray(options.imagePaths) && options.imagePaths.length === questions.length
            ? options.imagePaths
            : questions.map(() => "");
    ...
    onOpen() {
        // Allow the modal card to grow to fit an embedded image. The image
        // itself is capped (see renderStep), so this just lifts the default
        // narrow modal width when an image is present.
        if (this.modalEl) this.modalEl.style.maxWidth = "min(720px, 92vw)";
...
// Grid picker for choosing a vault image. `folderPath` is the gallery root —
// if empty, scans the whole vault. Recurses through subfolders. Calls
// `onPick(path)` when a thumbnail is clicked.
class DRImageGalleryModal extends Modal {
    constructor(app, folderPath, onPick) {
        super(app);
        this.folderPath = (folderPath || "").trim();
        this.onPick = onPick;
    }
    onOpen() {
        if (this.modalEl) this.modalEl.style.maxWidth = "min(820px, 92vw)";
        const { contentEl } = this;
        contentEl.empty();
        contentEl.createEl("h3", { text: "Choose image" });
        const desc = contentEl.createEl("p");
        desc.style.cssText = "color:var(--text-muted);font-size:0.9em;margin-bottom:12px;";
        desc.setText(this.folderPath ? `Browsing: ${this.folderPath}` : "Browsing entire vault.");

        const images = this.collectImages();
        if (!images.length) {
            const empty = contentEl.createEl("p");
            empty.style.cssText = "color:var(--text-muted);";
            empty.setText("No images found in this folder.");
            return;
        }
    ...
    collectImages() {
        ...
        const walk = (node) => {
            ...
            if (node instanceof TFile) {
                if (dr_isImageFile(node.name)) out.push(node);
                return;
            }
...
class DailyRitualModule {
    ...
    // Resolve a question's image attachment to a resource URL the modal can
    // drop into an <img src>. Returns "" when there's no image or the path
    // can't be resolved. Failure is silent — the modal just renders no image.
    async resolveQuestionImagePath(q) {
        if (!q || !q.imageMode || !q.imagePath) return "";
        try {
            if (q.imageMode === "static") {
                const f = this.app.vault.getAbstractFileByPath(q.imagePath);
                if (f && f instanceof TFile && dr_isImageFile(f.name)) {
            ...
            if (q.imageMode === "note") {
                ...
                const wl = content.match(/!\[\[([^\]|#^]+?)(?:[|#^][^\]]*)?\]\]/);
                if (wl) {
                    const dest = this.app.metadataCache.getFirstLinkpathDest(wl[1].trim(), f.path);
                    if (dest && dest instanceof TFile) return this.app.vault.getResourcePath(dest);
                }
                const md = content.match(/!\[[^\]]*\]\(([^)]+)\)/);
                if (md) {
                    const path = decodeURIComponent(md[1].trim());
                    const dest = this.app.vault.getAbstractFileByPath(path);
                    if (dest && dest instanceof TFile) return this.app.vault.getResourcePath(dest);
                }
                return "";
            }
            if (q.imageMode === "folder") {
                const folder = this.app.vault.getAbstractFileByPath(q.imagePath);
                if (!folder || !Array.isArray(folder.children)) return "";
                const images = folder.children.filter(c => c instanceof TFile && dr_isImageFile(c.name));
                if (!images.length) return "";
                images.sort((a, b) => (b.stat?.mtime || 0) - (a.stat?.mtime || 0));
                return this.app.vault.getResourcePath(images[0]);
            }
        } catch (e) {
            console.error("Daily Ritual: resolveQuestionImagePath failed", e);
        }
        return "";
    }

    async loadImagePathsFor(questions) {
        const out = [];
        for (const q of questions) out.push(await this.resolveQuestionImagePath(q));
        return out;
    }

    async resolveOneInjectedVar(q) {
        // Smart phase-gate — runs before (and independent of) injectVar.
        // Hides the question unless the chosen container's CURRENT phase
        // (its detector's `practice` token, computed for today) matches
        // phaseGatePhase. When in-phase, injects the prompt read from
        // phaseGateField on that container's current note.
        if (q && q.phaseGate && q.phaseGateContainerId && q.phaseGatePhase) {
    ...
    async openReflection() {
        const file = await this.resolveDailyRunTargetFile();
        if (!file) { new Notice("No active file open!"); return; }
        const resolved = await this.loadInjectedVarsFor(this.settings.questions);
        const imagePaths = await this.loadImagePathsFor(this.settings.questions);
        ...
        new DRReflectionModal(this.app, this.settings.questions, resolved, async (answers) => {
            ...
            this.settings.dailyDraft = null;
            await this.saveSettings();
        }, { initialAnswers, initialStep, onDraftChange, imagePaths }).open();
    }
    ...
    async openAlignment() {
        ...
        const imagePaths = await this.loadImagePathsFor(questions);
        let initialAnswers = null;
        let initialStep = 0;
    ...
    async processAlignment(file, answers, questions) {
        // Group output-to-field writes by their resolved target file, since
        // each question can name its own target note (blank = active file).
        // Grouping means multiple inline fields aimed at the same note are
        // applied to one content buffer in a single modify (no stale reads).
        const groups = new Map(); // path -> { file, items: [{ q, answer }] }
        for (let i = 0; i < questions.length; i++) {
            ...
            if (!answer) continue;
            const targetFile = await this.dr_resolveOutputTargetFile(file, q);
            if (!targetFile) continue; // notice already shown by resolver
            if (!groups.has(targetFile.path)) groups.set(targetFile.path, { file: targetFile, items: [] });
            groups.get(targetFile.path).items.push({ q, answer });
        }
        for (const { file: tf, items } of groups.values()) {
            ...
            if (touchedBody) await this.app.vault.modify(tf, content);
            const fmKeys = Object.keys(frontmatterUpdates);
            if (fmKeys.length > 0) {
                try {
                    await this.app.fileManager.processFrontMatter(tf, (fm) => {
    ...
    // Resolve where an alignment question's output field should be written.
    // Returns null (and shows a notice) when the configured target can't be
    // resolved, so the caller skips that answer rather than misfiling it.
    async dr_resolveOutputTargetFile(activeFile, q) {
        const mode = q.outputTargetMode || "active";
        if (mode === "active") return activeFile;
        if (mode === "note") {
            const path = (q.outputTargetNote || "").trim();
            if (!path) {
                new Notice(`Alignment: question has target "Specific note" but no path set — skipping that answer.`);
                return null;
            }
            let f = this.app.vault.getAbstractFileByPath(path);
            if (!(f instanceof TFile) && !path.toLowerCase().endsWith(".md")) {
                f = this.app.vault.getAbstractFileByPath(path + ".md");
            }
            if (f instanceof TFile) return f;
            new Notice(`Alignment: target note "${path}" not found — skipping that answer.`);
            return null;
        }
        if (mode === "container-current" || mode === "container-previous") {
            const id = q.outputTargetContainerId;
            const container = id ? this.findContainerById(id) : null;
            if (!container) {
                new Notice(`Alignment: no container picked for output target — skipping that answer.`);
                return null;
            }
            let f = null;
            if (mode === "container-current") {
                f = await this.findPRContainerCurrentNote(id);
            } else if (typeof this.plugin.findPreviousPRContainerNote === "function") {
                f = await this.plugin.findPreviousPRContainerNote(container);
            }
            if (f instanceof TFile) return f;
            new Notice(`Alignment: "${container.name}" has no ${mode === "container-current" ? "current" : "previous"} note — skipping that answer.`);
            return null;
        }
        if (mode === "recent-daily") {
            const f = this.findMostRecentDailyNote();
            if (f instanceof TFile) return f;
            new Notice(`Alignment: no daily note found for "Most recent daily note" target — skipping that answer.`);
            return null;
        }
        return activeFile;
    }

    // Newest date-named note in the Daily Notes core plugin's folder (the
    // whole vault when no folder is configured). Date is parsed from the
    // filename via dr_parseDateFromFilename, so the daily-note format must
    // be a date the parser recognizes. Returns null when none match.
    findMostRecentDailyNote() {
        const dailyPlugin = this.app.internalPlugins?.getPluginById?.("daily-notes");
        const opts = (dailyPlugin && dailyPlugin.instance && dailyPlugin.instance.options) || {};
        const folderPath = (opts.folder || "").trim();
        let files;
        if (folderPath) {
            const folder = this.app.vault.getAbstractFileByPath(folderPath);
            files = (folder && Array.isArray(folder.children)) ? folder.children : [];
        } else {
        ...
        for (const f of files) {
            ...
            const d = dr_parseDateFromFilename(f.name);
            if (!d) continue;
            const t = d.getTime();
    ...
    // Idempotent: creates today's daily note if it doesn't exist, using the
    // Daily Notes core plugin's configured folder / date format / template.
    // Writes via vault.create so Templater's "Trigger on new file creation"
    // hook fires without us having to open the file — keeps the user's focus
    // undisturbed at midnight. The post-create sleep gives Templater's async
    // hook time to expand <% %> tags before this function returns.
    async ensureTodaysDailyNote() {
        if (!this.settings.autoGenerateDailyNote) return;
        await this.getOrCreateTodaysDailyNote();
    }

    // Returns today's daily note as a TFile, creating it (Daily Notes core
    // plugin folder / format / template) if it doesn't exist. Returns null
    // when the Daily Notes plugin is unavailable or creation fails. Shared by
    // the midnight auto-create (ensureTodaysDailyNote) and the smart run-target
    // resolver (resolveDailyRunTargetFile).
    async getOrCreateTodaysDailyNote() {
        const dailyPlugin = this.app.internalPlugins?.getPluginById?.("daily-notes");
        if (!dailyPlugin || !dailyPlugin.enabled) return null;
        ...
        let body = "";
        const tplPathRaw = (opts.template || "").trim();
        if (tplPathRaw) {
            const tplPath = tplPathRaw.endsWith(".md") ? tplPathRaw : `${tplPathRaw}.md`;
            const tplFile = this.app.vault.getAbstractFileByPath(tplPath);
            if (tplFile && tplFile instanceof TFile) {
                try { body = await this.app.vault.read(tplFile); } catch (_) {}
            }
        }

        // Resolve core template tokens ({{title}}, {{date}}, {{time}},
        // {{yesterday}}, {{tomorrow}}) before write. These are the Obsidian
        // core-templates vocabulary that the Daily Notes / Periodic Notes
        // plugins normally resolve inside their own createDailyNote — since
        // we go around that to avoid stealing focus, we have to expand them
        // ourselves. Templater's <% %> tags are handled separately by its
        // file-create hook AFTER vault.create.
        const yesterday = today.clone().subtract(1, "day");
        const tomorrow = today.clone().add(1, "day");
        body = body
            .replace(/\{\{title\}\}/g, fileName)
            .replace(/\{\{date(?::([^}]+))?\}\}/g, (_, fmt) => today.format(fmt || format))
            .replace(/\{\{time(?::([^}]+))?\}\}/g, (_, fmt) => today.format(fmt || "HH:mm"))
            .replace(/\{\{yesterday(?::([^}]+))?\}\}/g, (_, fmt) => yesterday.format(fmt || format))
            .replace(/\{\{tomorrow(?::([^}]+))?\}\}/g, (_, fmt) => tomorrow.format(fmt || format));
        ...
        try {
            ...
            return created instanceof TFile ? created : (this.app.vault.getAbstractFileByPath(path) || null);
        } catch (e) {
            console.error("Daily Ritual: create today's daily note failed", e);
            const after = this.app.vault.getAbstractFileByPath(path);
            return after instanceof TFile ? after : null;
        }
    }

    // True when `file` looks like a daily note: inside the Daily Notes folder
    // (when one is configured) and with a filename that parses to a date.
    isDailyNoteFile(file) {
        ...
        return !!dr_parseDateFromFilename(file.name);
    }

    // Resolve which note a Daily Align / Daily Reflect run should write into.
    // With targetTodaysDailyNote off, it's just the active file (legacy). With
    // it on: the active note when that note is itself a daily note, otherwise
    // today's daily note (created if needed) — so a homepage button writes to
    // today while running inside yesterday's note writes to yesterday. Falls
    // back to the active file (with a notice) if today's note can't be made.
    async resolveDailyRunTargetFile() {
        const active = this.app.workspace.getActiveFile();
        if (!this.settings.targetTodaysDailyNote) return active;
        if (active && this.isDailyNoteFile(active)) return active;
        const today = await this.getOrCreateTodaysDailyNote();
        if (today instanceof TFile) return today;
        new Notice("Daily Ritual: couldn't resolve today's daily note (is the Daily Notes core plugin enabled?). Using the active note.");
        return active;
    }
    ...
    renderSettingsTab(containerEl) {
        ...
        new Setting(containerEl)
            .setName("Image gallery folder")
            .setDesc("Folder browsed by the image picker and used as the upload destination. Leave blank to browse the entire vault (uploads then land in vault root).")
            .addButton((btn) => {
                btn.setButtonText(this.settings.imageGalleryFolder ? "Change" : "Choose").onClick(() => {
                    new FolderSuggestModal(this.app, async (folder) => {
                        this.settings.imageGalleryFolder = folder.path;
                        await this.saveSettings();
                        this.plugin.settingTab.display();
                    }).open();
                });
            ...
            .addExtraButton((btn) => {
                btn.setIcon("cross").setTooltip("Clear").onClick(async () => {
                    this.settings.imageGalleryFolder = "";
                    await this.saveSettings();
                    this.plugin.settingTab.display();
                });
            });
        const galleryStatus = containerEl.createEl("p");
        galleryStatus.style.cssText = "color:var(--text-muted);font-size:0.85em;margin:0 0 16px 4px;";
        galleryStatus.setText(`Currently: ${this.settings.imageGalleryFolder || "(vault root)"}`);

        containerEl.createEl("h3", { text: "Auto-generate" });
        new Setting(containerEl)
            .setName("Auto-generate daily note at midnight")
            .setDesc("When on, creates today's daily note in the background once the day boundary crosses, using the Daily Notes core plugin's folder, date format, and template. Templater scripts fire via its 'Trigger on new file creation' hook. Requires the Daily Notes core plugin to be enabled. Toggle takes effect after reloading the plugin.")
            .addToggle((t) => {
                t.setValue(!!this.settings.autoGenerateDailyNote).onChange(async (v) => {
                    this.settings.autoGenerateDailyNote = v;
                    await this.saveSettings();
                });
            });
        new Setting(containerEl)
            .setName("Run against today's daily note")
            .setDesc("When on, launching Daily Align or Daily Reflect from a note that isn't a daily note writes to today's daily note (created if needed). Launched from inside a daily note, that note is used. Lets a homepage button write to today instead of the homepage. Requires the Daily Notes core plugin.")
            .addToggle((t) => {
                t.setValue(!!this.settings.targetTodaysDailyNote).onChange(async (v) => {
                    this.settings.targetTodaysDailyNote = v;
                    await this.saveSettings();
                });
            });
    ...
    renderQuestions(container, questions, expandKey, bankType) {
        ...
        const expIm = (this.expandedImage[expandKey] = this.expandedImage[expandKey] || {});
        ...
        questions.forEach((q, i) => {
            ...
            setting.addExtraButton((btn) => {
                btn.setIcon("image").setTooltip("Attach an image to this question").onClick(() => {
                    expIm[i] = !expIm[i];
                    rerender();
                });
            });
            ...
            if (expIn[i]) {
                ...
                // phase's prompt from the container's current note.
                new Setting(inputGroup)
                    .setName("Phase-gated (smart)")
                    .setDesc("Show this question only while the chosen container is in a given phase; injects that phase's prompt from the container's current note.")
                    .addToggle((t) => {
                        t.setValue(!!q.phaseGate).onChange(async (v) => {
                            q.phaseGate = v; await this.saveSettings(); rerender();
                        });
                    });
                if (q.phaseGate) {
                    new Setting(inputGroup)
                        .setName("Container")
                        .addDropdown((dd) => {
                            dd.addOption("", "— pick a container —");
                            for (const c of this.listPRContainers()) dd.addOption(c.id, c.name || "(unnamed)");
                            dd.setValue(q.phaseGateContainerId || "").onChange(async (v) => {
                                q.phaseGateContainerId = v; await this.saveSettings();
                            });
                        });
                    new Setting(inputGroup)
                        .setName("Show during phase")
                        .setDesc("Matches the container detector's `practice` token (e.g. training / visual / via_negativa / detach).")
                        .addText((t) => {
                            t.setPlaceholder("e.g. training").setValue(q.phaseGatePhase || "")
                ...
                if ((q.responseMode || "input") === "prompt") {
                    ...
                    if (q.outputToField) {
                        ...
                        if (bankType !== "reflection") {
                            ...
                            new Setting(outputGroup)
                                .setName("Target note")
                                .setDesc("Which note this answer is written into.")
                                .addDropdown((dd) => {
                                    dd.addOption("active", "Active note");
                                    dd.addOption("note", "Specific note");
                                    dd.addOption("container-current", "Container note (current)");
                                    dd.addOption("container-previous", "Container note (previous)");
                                    dd.addOption("recent-daily", "Most recent daily note");
                                    dd.setValue(mode).onChange(async (v) => {
                                        q.outputTargetMode = v; await this.saveSettings(); rerender();
                                    });
                                });
                            if (mode === "note") {
                                new Setting(outputGroup)
                                    .setName("Note path")
                                    .setDesc("Vault path of the note to write into.")
                                    .addText((t) => {
                                        t.setPlaceholder("e.g. Journals/Goals.md").setValue(q.outputTargetNote || "")
                                            .onChange(async (v) => { q.outputTargetNote = v.trim(); await this.saveSettings(); });
                                    });
                            } else if (mode === "container-current" || mode === "container-previous") {
                                new Setting(outputGroup)
                                    .setName("Container")
                                    .setDesc("Which Periodic Ritual container's note to write into.")
                                    .addDropdown((dd) => {
                                        dd.addOption("", "— pick a container —");
                                        for (const c of this.listPRContainers()) dd.addOption(c.id, c.name || "(unnamed)");
                                        dd.setValue(q.outputTargetContainerId || "")
                                            .onChange(async (v) => { q.outputTargetContainerId = v; await this.saveSettings(); });
                                    });
                            }
                        }
                    }
            ...
            if (expIm[i]) {
                const imageGroup = container.createDiv({ cls: "daily-log-nested-settings" });
                imageGroup.style.cssText = "padding-left:24px;border-left:2px solid var(--interactive-accent);margin-bottom:12px;";
                this.renderQuestionImagePanel(imageGroup, q, rerender);
            }
        });
    }

    // Renders the per-question image config inside the expanded panel.
    // Mode controls how `imagePath` is interpreted at modal-render time.
    renderQuestionImagePanel(group, q, rerender) {
        new Setting(group)
            .setName("Image mode")
            .setDesc("Static — fixed image file. From note — first embedded image in a .md note. From folder — most recently modified image in a folder.")
            .addDropdown((dd) => {
                dd.addOption("", "None");
                dd.addOption("static", "Static image file");
                dd.addOption("note", "From note (first embedded image)");
                dd.addOption("folder", "From folder (latest image)");
                dd.setValue(q.imageMode || "").onChange(async (v) => {
                    q.imageMode = v;
                    q.imagePath = "";
                    await this.saveSettings();
                    rerender();
        ...
        if (!q.imageMode) return;

        const mode = q.imageMode;
        const pathSetting = new Setting(group).setName("Image path");
        pathSetting.setDesc(q.imagePath || "No image selected.");

        if (mode === "static") {
            pathSetting.addButton((btn) => {
                btn.setButtonText(q.imagePath ? "Change" : "Choose").onClick(() => {
                    new DRImageGalleryModal(this.app, this.settings.imageGalleryFolder || "", async (picked) => {
                        q.imagePath = picked;
                        await this.saveSettings();
                        rerender();
                    }).open();
            ...
            pathSetting.addButton((btn) => {
                btn.setButtonText("Upload").setTooltip("Pick an image from disk; it'll be copied into the image gallery folder.").onClick(() => {
                    this.runImageUpload().then(async (newPath) => {
                        if (!newPath) return;
                        q.imagePath = newPath;
                        await this.saveSettings();
                        rerender();
            ...
            pathSetting.addButton((btn) => {
                btn.setButtonText(q.imagePath ? "Change note" : "Choose note").onClick(() => {
                    new MarkdownFileSuggestModal(this.app, async (file) => {
                        q.imagePath = file.path;
                        await this.saveSettings();
                        rerender();
                    }).open();
                });
            ...
            pathSetting.addButton((btn) => {
                btn.setButtonText(q.imagePath ? "Change folder" : "Choose folder").onClick(() => {
                    new FolderSuggestModal(this.app, async (folder) => {
                        q.imagePath = folder.path;
                        await this.saveSettings();
                        rerender();
                    }).open();
                });
        ...
        // Live preview — resolves the configured source and shows the result.
        this.resolveQuestionImagePath(q).then((src) => {
            if (!src) return;
            const preview = group.createDiv();
            preview.style.cssText = "padding:8px 0 4px;";
            const img = preview.createEl("img");
            img.src = src;
            img.style.cssText = "max-width:240px;max-height:160px;height:auto;border-radius:4px;border:1px solid var(--background-modifier-border);display:block;";
            img.addEventListener("error", () => { preview.remove(); });
        });
    }

    // Opens an OS file picker, copies the selected image into the image
    // gallery folder (creating it if needed), and returns the new vault path.
    // Resolves to "" if the user cancels or anything fails.
    async runImageUpload() {
        return new Promise((resolve) => {
            ...
            input.addEventListener("change", async () => {
                ...
                if (!dr_isImageFile(file.name)) {
                    new Notice("Daily Ritual: not a recognized image file.");
                    resolve(""); return;
                }
                try {
                    const folder = (this.settings.imageGalleryFolder || "").trim();
                    if (folder && !this.app.vault.getAbstractFileByPath(folder)) {
                    ...
                    let dest = folder ? `${folder}/${baseName}` : baseName;
                    let n = 1;
                    while (this.app.vault.getAbstractFileByPath(dest)) {
                        dest = folder ? `${folder}/${stem}-${n}.${ext}` : `${stem}-${n}.${ext}`;
                        n++;
                    }
                    const ab = await file.arrayBuffer();
                    await this.app.vault.createBinary(dest, ab);
                    new Notice(`Uploaded to ${dest}`);
                    resolve(dest);
                } catch (e) {
                    console.error("Daily Ritual: image upload failed", e);
                    new Notice("Daily Ritual: image upload failed (see console).");
                    resolve("");
                }
            }, { once: true });
...
class MonthlyRitualPlugin extends Plugin {
    ...
    async loadSettings() {
        ...
        const dr_migrateQuestion = (q) => {
            ...
            if (q.phaseGate === undefined) q.phaseGate = false;
            if (q.phaseGateContainerId === undefined) q.phaseGateContainerId = "";
            if (q.phaseGatePhase === undefined) q.phaseGatePhase = "";
            if (q.phaseGateField === undefined) q.phaseGateField = "";
        };
        ...
        const dr_isEmptyDefaultQuestion = (q) => (
            ...
            && (q.outputTargetMode || "active") === "active"
            && !q.outputTargetNote && !q.outputTargetContainerId
...
const re = new RegExp(`^${escapeRegex(question.varField)}::[ \\t]*(.+)$`, "m");
    ...
    async runPRContainerReflection(container) {
        ...
        new ReflectionModal(this.app, reflection.questions, injectedVars, async (answers) => {
            ...
            // Stamp the note so the Reflect queue knows this period's
            // reflection is done. Lives in the note's PR blob (not
            // data.json) so it syncs across devices, mirroring the
            // writeback marker. Delete the `reflected` key from the blob
            // to re-queue a period.
            await this.updatePRMetadataOnFile(file, container, {
                reflected: "true",
...
class RitualCalendarView extends ItemView {
    ...
    // One-shot timer scheduled at the end of each render(). When local midnight
    // arrives, re-render so the "today" outline moves to the new day without a
    // plugin reload. Re-armed inside the re-render. Cleared in onClose.
    scheduleMidnightRefresh() {
        ...
        this._midnightTimer = window.setTimeout(() => {
            ...
            if (this.rootEl && this.rootEl.isConnected) {
                this.render().catch(e => console.error("Zodiac Calendar: midnight refresh failed", e));
            }
        }, ms);
    }
